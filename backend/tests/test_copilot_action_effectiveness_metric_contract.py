...
def test_minutes_metric_keeps_minutes_wording():
    ...
    assert "highest historical realized minutes saved" in response["answer"]
    assert "70.0% SLA success rate" in response["answer"]


def test_sla_metric_uses_sla_wording_and_sla_ranking():
    plan = WarehouseQueryPlan(
        intent="action_effectiveness",
    ...
    assert "13.0 average realized minutes saved" in response["answer"]
