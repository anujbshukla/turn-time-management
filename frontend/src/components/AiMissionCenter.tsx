...
type MissionPreference = {
  ...
  updatedAt: string;
  databaseMissionId?: number | null;
};

const STORAGE_KEY =
  "warehouse-ai-mission-preferences";
...
export function AiMissionCenter({
  ...
  const [preferences, setPreferences] =
    useState<
      Record<string, MissionPreference>
    >({});
  ...
  const [
    scenarioMissionId,
    setScenarioMissionId,
  ] = useState<string | null>(null);

  const [
    scenarioLoadingId,
    setScenarioLoadingId,
  ] = useState<string | null>(null);
  ...
  async function runMissionScenario(
    mission: AiMission,
  ) {
    if (
      !mission.facility_id ||
      ...
      setMissionError(
        "Mission operating-window context is incomplete.",
      );
      return;
    }
    ...
    try {
      const result =
        await simulateOptimizationMission({
          facility_id:
            mission.facility_id,
          date_from:
            mission.window_start,
  ...
  async function acceptMission(
    ...
    try {
      const execution =
        await acceptOptimizationMission(
          mission,
          scenarioResults[
            mission.mission_id
          ]
  ...
  async function transitionMission(
    ...
    try {
      const updated =
        await updateOptimizationMissionStatus(
          databaseMissionId,
          status,
        );
  ...
  async function refreshMissionOutcomes(
    mission: AiMission,
  ) {
    const execution =
      executions[
    ...
    if (!databaseMissionId) {
      setMissionError(
        "This mission has not been persisted yet.",
      );
      return;
    }
    ...
    try {
      const updated =
        await refreshOptimizationMissionOutcomes(
          databaseMissionId,
        );
                  ...
                  <div className="ai-mission-content">
                    <div className="ai-mission-card-heading">
                      <div>
                        <div className="ai-mission-badges">
                          <span
                            className={`mission-severity ${mission.severity.toLowerCase()}`}
                          >
                            {
                              mission.severity
                            }
                          ...
                          <span
                            className={`mission-status ${mission.effectiveStatus.toLowerCase()}`}
                          >
                            {
                              mission.effectiveStatus
                            }
                    ...
                    <div className="ai-mission-metrics">
                      <div>
                        ...
                        <strong>
                          {mission.impacted_appointment_count.toLocaleString()}
                        </strong>
                      </div>
                      ...
                      <div>
                        ...
                        <strong>
                          {Math.round(
                            mission.projected_minutes_saved,
                          ).toLocaleString()}
                        </strong>
                      </div>
                    ...
                    <div className="ai-mission-actions-list">
                      {mission.recommended_actions.map(
                        (action) => (
                          <span
                            key={
                              action
                        ...
                        .map(
                          (item) => (
                            ...
                            >
                              ...
                              <b>
                                {Math.round(
                                  item.baseline_projected_turn_minutes,
                                )}
                                {" → "}
                                {Math.round(
                                  item.optimized_projected_turn_minutes,
                                )}
                                {
                                  " min"
                                }
